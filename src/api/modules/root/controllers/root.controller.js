...
class RootController {
    ...
    async bucket({ app, headers, params, query }, res) {
        try {
            ...
            if (data) {
                if (query.type) {
                    if (query.type === 'folder') {
                        ...
                        app.locals.winston.info({ bucket: params.bucket, message: 'Bucket search' }, { agent: headers['user-agent'] });
            ...
            app.locals.winston.error({ error, message: 'Bucket search' }, { agent: headers['user-agent'] });
            ...
            fs.readdir(`./data/${params.bucket}/${params.param}`, (err, data) => {
                if (err) {
                    app.locals.winston.info(
                        { bucket: params.bucket, folder: params.param, message: 'Bucket not found (folder list)' },
