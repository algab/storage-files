            ...
            if (user.length == 0) {
                ...
                fs.rename(`./data/${name}`, `./data/${req.body.name}`, async (err) => {
                    if (err) {
                        ...
                        await this.db.run("UPDATE buckets SET name = ?, private = ? WHERE owner = ?", [req.body.name, Boolean(req.body.private), req.body.nick]);
    ...
    async sizeBucket(name) {
        ...
        for (let i = 0; i < data.length; i++) {
            if (data[i].search(new RegExp("[.]")) == -1) {
                size += await this.sizeFolder(name, data[i]);
            }
            else {
